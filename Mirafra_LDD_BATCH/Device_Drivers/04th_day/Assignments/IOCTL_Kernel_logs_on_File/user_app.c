...
int main() {
    ...
    // Open the device
    fd = open("/dev/kernel_logs", O_RDWR);
    if (fd < 0) {
    ...
    printf(" ^ : Power (num1 raised to num2)\n");
    printf(" %% : Modulo (num1 %% num2)\n");
    printf(" g : GCD (Greatest Common Divisor)\n");
    printf(" l : LCM (Least Common Multiple)\n\n");
    ...
    // Output the result
    printf("Operation: %d %c %d = %d\n", input_data.num1, input_data.operation, input_data.num2, result);
